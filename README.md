...
| 11 — member-dispatch mount | Crew member-DM parity: a `kirocrew-dashboard`-shaped per-session MCP entry (`type:"stdio"`, env array with `KIROCREW_SESSION_KEY`+`KIROCREW_BOUND_PORT`) rides `session/new`'s mount through the permission gate (`mode=read-only` — dispatch verbs ASK, never pre-approved). No adapter gap: tagged-stdio (`mcp-bridge.ts` `serverKind`), env merge (`envArrayToRecord` over `process.env`), `mcp__kirocrew-dashboard__*` naming, and `kindForTool→other` all passed it through unchanged. Scoped to `session/new` (`session/load` re-declaration is slice 10's) | ✅ PASS — `test/slice11-member-dispatch.mjs` + `test/dispatch-mcp-server.mjs` stub (tagged-stdio listed, ASK once-allowed, byte-identical key+port in result, `mode=read-only` held) |
